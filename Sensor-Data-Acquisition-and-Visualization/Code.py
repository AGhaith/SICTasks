...
#    ultrasonic = DistanceSensor(echo=17, trigger=27)
    ...
    def log() :
        logp = open("log.txt",'a')
        status = f"{datetime.now().strftime('%Y-%B-%d\t%A %I:%M:%S %p')}The Current Temperature is {temperature}CThe Current Humidity is {humidity}%The Current Distance in meter is {dim}"
        logp.write(status)
        logp.close()
    ...
    def updatevariables():
        ...
        dim = None #int(ultrasonic.distance * 100) 
        humidity = 5  
        temperature = 2
        dim = 10
